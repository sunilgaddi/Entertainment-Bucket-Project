            ...
            <div className='movie__row__posters'>
                {
                    data ? data.map((item, id) => {
                        return <Link key={id} to={`/eb/home/${mainPanel}/${item.original_title || item.original_name}/${item.id}`}><div className='movie__row__poster' style={{ backgroundImage: {mainPanel === 'sports' ? ${item?.banner} : "https://image.tmdb.org/t/p/original/${item?.poster_path}})`, backgroundPosition: 'center', backgroundSize: "cover" }} >{item.poster_path === null && 'N/A'}</div>{subPanel === 'season' && <span>Season {item?.season_number}</span>}</Link>
                    })
                        :
                        ""
                }
